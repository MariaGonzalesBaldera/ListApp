...
import {Swipe, UserRematch, UserSwipe} from '../../../Services/meet/MeetServices';
...
export default function DetailProfilemsg() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const userData = await UserRematch(); // Aquí asumes que `MeUser` es una función que obtiene datos del usuario
        setSwipe(userData);
  ...
  return (
    <GestureHandlerRootView style={{flex: 1}}>
      <View style={{flex: 1, backgroundColor: theme.background}}>
        ...
        />      
        { swipe.length > 0 ? ( <ScrollView style={{paddingHorizontal: 20}}>
          {/* Perfil con gestos */}
          <PanGestureHandler onGestureEvent={gestureHandler}>
            <Animated.View style={[{marginVertical: 10}, animatedStyle]}>
              <LinearGradient
                colors={['rgba(128, 128, 128, 0.6)', 'rgba(128, 128, 128, 0.6)']}
        ...
        <View style={[styles.noChatsContainer, {justifyContent:'center', paddingHorizontal:20}]}>
                <Text style={[styles.noChatsText, fonts.H3,{color:theme.text}]}>
                  You don't have any profile to see
                </Text>
              </View></>}
      </View>
    </GestureHandlerRootView>
